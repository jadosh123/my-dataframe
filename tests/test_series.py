...
class TestSeries:
    ...
    def test_loc_list(self, my_int_srs, pd_int_srs):
        assert str(my_int_srs.loc[[2, 3, 4, 4]]) == str(pd_int_srs.loc[[2, 3, 4, 4]])

    def test_loc_slice(self, my_int_srs, pd_int_srs):
        assert str(my_int_srs.loc[2:4]) == str(pd_int_srs.loc[2:4])
    ...
    def test_nan_lbl_val_types(self, my_nan_srs: Series, pd_nan_srs: pd.Series):
        assert [my_nan_srs.index.dtype, my_nan_srs.values.dtype] == [
            pd_nan_srs.index.dtype,
            pd_nan_srs.values.dtype,
        ]

    def test_nan_addition(self, my_nan_srs: Series, pd_nan_srs: pd.Series):
        npt.assert_array_equal(
            (my_nan_srs + my_nan_srs).values,
            (pd_nan_srs + pd_nan_srs).values,
        )

    def test_nan_mult(self, my_nan_srs: Series, pd_nan_srs: pd.Series):
        npt.assert_array_equal(
            (my_nan_srs * my_nan_srs).values,
            (pd_nan_srs * pd_nan_srs).values,
        )

    def test_nan_truediv(self, my_nan_srs: Series, pd_nan_srs: pd.Series):
        npt.assert_array_equal(
            (my_nan_srs / my_nan_srs).values,
            (pd_nan_srs / pd_nan_srs).values,
        )

    def test_nan_addition_labels(self, my_nan_srs: Series, pd_nan_srs: pd.Series):
        npt.assert_array_equal(
            (my_nan_srs + my_nan_srs).index,
            (pd_nan_srs + pd_nan_srs).index,
        )

    def test_nan_mult_labels(self, my_nan_srs: Series, pd_nan_srs: pd.Series):
        npt.assert_array_equal(
            (my_nan_srs * my_nan_srs).index,
            (pd_nan_srs * pd_nan_srs).index,
        )

    def test_nan_truediv_labels(self, my_nan_srs: Series, pd_nan_srs: pd.Series):
        npt.assert_array_equal(
            (my_nan_srs / my_nan_srs).index,
            (pd_nan_srs / pd_nan_srs).index,
        )
